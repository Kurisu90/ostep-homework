...
class cpu:
    ...
    #
    # 計算有效位址（LOAD EFFECTIVE ADDRESS，除了最後不會真的存取記憶體）
    #
    ...
    def getregname(self, r):
        ...
        if t == '':
            return 'zero'
        zassert(t[0] == '%', '應該要是合法的暫存器名稱，但收到的是 [%s]' % r)
    ...
    def getarg(self, arg):
        ...
        if tmp[0] == '$':
            ...
            # 最一般的情況：number(reg,reg) 或 number(reg) 或 number(reg,reg,number)
            neg = 1
            if tmp[0] == '-':
                tmp = tmp[1:]
                neg = -1
            s = tmp.split('(')
            if len(s) == 1:
                # 沒有括號 -> 直接假設這是一個常數值（也就是位址），例如 mov 10, %ax
                value = neg * int(tmp)
                return '%d,%d,%d,1' % (int(value), self.register_translate('zero'), self.register_translate('zero')), 'TYPE_MEMORY'
            elif len(s) == 2:
                # 這裡直接假設括號內是我們要處理的內容
                # 例如 mov 10(%ax) 或 mov 10(%ax,%bx) 或 mov 10(%ax,%bx,10) 或 mov (%ax,%bx,10) 等等

                # 如果前面沒有數字，第一個字元就應該是左括號；這種情況下數值直接設為 0
                # 否則就要處理一個數字或負數
                if tmp[0] != '(':
                    zassert(s[0].strip().isdigit() == True, '開頭的數字應該是數字 [%s]' % s[0])
                    value = neg * int(s[0])
                else:
                    value = 0
    ...
    def removecommas(self, cline, inargs):
        ...
        for i in range(len(inargs)):
            if inargs[i] == '(':
                zassert(inparen == False, '引數中不能有巢狀括號 [%s]' % cline)
                inparen = True
            if inargs[i] == ')':
                zassert(inparen == True, '右括號前必須先有左括號 [%s]' % cline)
                inparen = False
            if inparen == True:
                if inargs[i] == ',':
                    outargs += '__BREAK__'
                ...
                outargs += inargs[i]
        zassert(inparen == False, '括號沒有閉合 [%s]' % cline)
        return outargs
        ...
        for line in fd:
            ...
            if cline[0] != '.':
                ...
                if self.verbose == True:
                    print('作業碼(opcode)', opcode)

                # 主要的作業碼（OPCODE）處理迴圈
                if opcode == 'mov':
                    # 這是最難解析的一種（因為格式很通用）
                    # 可能是 mov x(r1,r2,4), r3 或 mov r1, (r2,r3) 等等
                    outargs = self.removecommas(cline, tmp[1])

                    rtmp = outargs.split(',')
                    zassert(len(rtmp) == 2, 'mov: 需要兩個以逗號分隔的引數 [%s]' % cline)
                    arg1 = rtmp[0].strip()
                    arg2 = rtmp[1].strip()
                    ...
                    rtmp = tmp[1].split(',', 1)
                    zassert(len(tmp) == 2 and len(rtmp) == 2, 'lea: 需要兩個以逗號分隔的引數 [%s]' % cline)
                    arg1 = rtmp[0].strip()
                    arg2 = rtmp[1].strip()
                    (src, stype) = self.getarg(arg1)
                    (dst, dtype) = self.getarg(arg2)
                    if stype == 'TYPE_MEMORY' and dtype == 'TYPE_REGISTER':
                        ...
                        zassert(False, 'lea 指令格式錯誤（來源應為記憶體位址，目的應為暫存器）')
                elif opcode == 'neg':
                    zassert(len(tmp) == 2, 'neg: 只能接受一個引數')
                    arg = tmp[1].strip()
                    (dst, dtype) = self.getarg(arg)
                    zassert(dtype == 'TYPE_REGISTER', 'neg 只能對暫存器操作')
                    ...
                    rtmp = tmp[1].split(',', 1)
                    zassert(len(tmp) == 2 and len(rtmp) == 2, 'mul: 需要兩個以逗號分隔的引數 [%s]' % cline)
                    arg1 = rtmp[0].strip()
                    arg2 = rtmp[1].strip()
                    (src, stype) = self.getarg(arg1)
                    (dst, dtype) = self.getarg(arg2)
    ...
    def run(self, procs, intfreq, intrand):
        ...
        while True:
            ...
            # 處理中斷
            # 只是把中斷倒數計數器往下數到 0
            # 當它歸零時，或發出 'yield' 指令時（rc=-2）
            # 就會發生一次切換
            # 關鍵：若採用手動排程（procsched），中斷必須每道指令都發生，
            # 這樣手動排程才會正確運作
            interrupt -= 1
...
class proclist:
    def __init__(self):
        self.plist     = []      # 行程物件清單
        self.active    = 0       # 追蹤目前有多少個行程仍在活動中
        self.manual    = False
        self.procsched = []      # 各行程執行順序的清單（依 ID 排列）
        self.curr      = 0       # 目前正在執行的行程（procsched 清單中的索引）
    ...
    def finalize(self, procsched):
        ...
        if len(check) != self.active:
            print('不合法的排程：未包含所有行程', self.procsched)
            exit(1)
        self.curr = 0
        self.restore()
        return
...
parser.add_option('-P', '--procsched', default='',         help='精確控制哪個執行緒在何時執行',
...
parser.add_option('-H', '--headercount',default=-1,        help='多久重印一次表頭',                  action='store',      type='int',    dest='headercount')
...
print('參數 手動排程(procsched)',             options.procsched)
